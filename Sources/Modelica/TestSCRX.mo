package TestSCRX
  ...
  package Components "Package of components"
    ...
    model SCRX "Static Excitation Control System"
    ...
    equation
      ...
      connect(EFD, product.y) annotation (
        Line(points={{110,0},{97,0},{97,-60},{94.4,-60}},        color = {0, 0, 127}));
      connect(product.u2, VoltageReference.y) annotation (
        Line(points={{85.2,-62.4},{85.2,-80.4},{-73.8,-80.4},{-73.8,-34},{-79,
              -34}},                                                                      color = {0, 0, 127}));
      connect(limiter.y, product.u1) annotation (
        Line(points={{80.4,-58},{82,-58},{82,-57.6},{85.2,-57.6}},
                                              color = {0, 0, 127}));
      ...
      connect(feedback1.u1, gain111.y) annotation (
        Line(points={{40,-1},{28,-1},{28,-1.4},{14,-1.4}},
                                            color = {0, 0, 127}));
      ...
      connect(add.u1, gain2.y) annotation (
        Line(points={{-18.8,-64.6},{2,-64.6},{2,-65},{23.4,-65}},
                                                       color = {0, 0, 127}));
      connect(add.y, feedback2.u2) annotation (
        Line(points={{-32.6,-61},{-39.0625,-61},{-39.0625,-35},{-40,-35}},            color = {0, 0, 127}));
      ...
      connect(gain1.y, add.u2) annotation (
        Line(points={{-10.6,-57},{-14,-57},{-14,-57.4},{-18.8,-57.4}},
                                                    color = {0, 0, 127}));
      ...
      connect(feedback2.u1, feedback.y) annotation (
        Line(points={{-48,-27},{-50,-27},{-50,-26},{-51,-26}},
                                                color = {0, 0, 127}));
    ...
    model SCRXwInitAsInput
      "Static Excitation Control System with Initialization values as Input"
      parameter Real T_AT_B = 0.1 "Ratio between regulator numerator (lead) and denominator (lag) time constants";
      parameter Real T_B = 1 "Regulator denominator (lag) time constant";
      parameter Real K = 100 "Excitation power source output gain";
      parameter Real T_E = 0.005 "Excitation power source output time constant";
    ...
    //  parameter Real ECOMP0 = 1;
      ...
      Modelica.Blocks.Math.Gain gain1(k = a1) annotation (
        Placement(transformation(origin = {-4, -57}, extent = {{-6, -6}, {6, 6}}, rotation = 180)));
      Modelica.Blocks.Interfaces.RealInput EFD0 "Initial value of field voltage"
        annotation (Placement(transformation(
            ...
            rotation=90,
            origin={-60,-120})));
      Modelica.Blocks.Interfaces.RealInput ECOMP0 "Initial value of the exciter input" annotation (Placement(
            transformation(
            extent={{-20,-20},{20,20}},
            rotation=90,
            origin={40,-120}), iconTransformation(
    ...
    equation
      ...
      connect(XADIFD, sCRX.XADIFD) annotation (Line(points={{-100,-60},{75.6,-60},{
              75.6,-35.2}},
                       color={0,0,127}));
      connect(const.y, sCRX.VOTHSG) annotation (Line(points={{-39,30},{-24,30},
              {-24,14},{-6,14},{-6,12.8},{14.8,12.8}},
                                                  color={0,0,127}));
      ...
      connect(EFD0, sCRX.EFD0) annotation (Line(points={{-100,60},{-20,60},{-20,
              -12.8},{14.8,-12.8}}, color={0,0,127}));
      connect(ECOMP, sCRX.ECOMP)
        annotation (Line(points={{-100,0},{14.8,0}}, color={0,0,127}));
      annotation (
  ...
  package Experiments
    ...
    model TestWithOpenIPSLInitAsParam
      "Tests the SCRX model that does not have inputs for initial values, but parameters instead"
      extends OpenIPSL.Tests.BaseClasses.SMIB(pwFault(t1=12, t2=12.5));
    OpenIPSL.Electrical.Machines.PSSE.GENROU gENROU(D = 0, H = 4.28,
        M_b=100000000,
        P_0=40000000,
